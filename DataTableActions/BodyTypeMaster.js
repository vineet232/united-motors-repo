...
var show = false;
 ...
 var buttons = [''];
  ...
  if (show) {
    ...
    $.ajax({
        type: "POST",
        url: "masterbodytype.aspx/GetAllData",
        data: '',
...
function AddRecord() {
    ...
    var bodytype = $('[id*=txtName]').val();
   // name = sentenceCase(name);
    var model = {};
    model.bodytype = bodytype;
    $.ajax({
        type: "POST",
        url: "masterbodytype.aspx/SaveRecord",
        ...
        success: function (r) {
            ...
            if (r.d == 10) {
                ...
                if (r.d == 20 || r.d == 30) {
                    if (r.d == 20) {
                        $.notify("Error!! body type already exists!", "error");
                        return;
                    }
                    if (r.d == 30) {
                        $.notify("Error!! body type cannot be blank!", "error");
                        $('[id*=txtName]').focus();
                        return;
                    }
                }
...
function EditRecordbyID(id) {
    ...
    $.ajax({
        type: "POST",
        url: "masterbodytype.aspx/GetRecordbyID",
        data: '{ id :' + id + '}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (result) {
            ...
            if (result.d == "10") {
                ...
                var bodytype = json[0].bodytype;
                //alert(modelname);
                $('#recordID').val(json[0].id);
                $('#txtName').val(bodytype);
...
function UpdateRecord() {
    ...
    if (id > 0) {
        ...
        $.ajax({
            type: "POST",
            url: "masterbodytype.aspx/UpdateRecord",
            data: '{ model :' + JSON.stringify(model) + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function DeleteRecord(id) {
    if (confirm('Are you sure delete this record?')) {
        ...
        $.ajax({
            type: "POST",
            url: "masterbodytype.aspx/DeleteRecord",
            data: '{ id :' + id + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
