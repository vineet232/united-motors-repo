...
function GetData() {
    ...
    $.ajax({
        type: "POST",
        url: "masterPaymentMethodwithBank.aspx/GetAllData",
        data: '',
...
function AddRecord() {
    ...
    var paymentcode = $('[id*=txtpaymentcode]').val();
    var isbanktype = "";
    if ($('#chkisbanktype').prop('checked')) {
        isbanktype = "1";
    } else {
        isbanktype = "0";
    }
   // name = sentenceCase(name);
    ...
    model.isbanktype = isbanktype;
    model.paymentcode = paymentcode;

    $.ajax({
        type: "POST",
        url: "masterPaymentMethodwithBank.aspx/SaveRecord",
...
function EditRecordbyID(id) {
    ...
    $.ajax({
        type: "POST",
        url: "masterPaymentMethodwithBank.aspx/GetRecordbyID",
        data: '{ id :' + id + '}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (result) {
            ...
            if (result.d == "10") {
                ...
                $('#txtpaymentcode').val(json[0].paymentcode);



                var isbanktype = json[0].isbanktype;
                if (isbanktype == "1") {
                    /////alert(json[0].isbanktype);
                    $('#chkisbanktype').prop('checked');
                } else {
                    $("#chkisbanktype").removeAttr("checked");
                }
...
function UpdateRecord() {
    ...
    if (id > 0) {
        ...
        $.ajax({
            type: "POST",
            url: "masterPaymentMethodwithBank.aspx/UpdateRecord",
            data: '{ model :' + JSON.stringify(model) + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function DeleteRecord(id) {
    if (confirm('Are you sure delete this record?')) {
        ...
        $.ajax({
            type: "POST",
            url: "masterPaymentMethodwithBank.aspx/DeleteRecord",
            data: '{ id :' + id + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
...
function validate() {
    ...
    if ($('#txtpaymentcode').val().trim() == "") {
        $('#txtpaymentcode').css('border-color', 'Red');
        isValid = false;
    }
    else {
        $('#txtpaymentcode').css('border-color', 'lightgrey');
    }
