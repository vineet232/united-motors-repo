...
var show = false;
 ...
 var buttons = [''];
  ...
  if (show) {
    ...
    $.ajax({
        type: "POST",
        url: "premiumtpamaster.aspx/GetAllData",
        data: '',
...
function AddRecord() {
    ...
    var gvrange = $('[id*=txtName]').val();
    var gvrate = $('[id*=txtgvrate]').val();
   // name = sentenceCase(name);
    var model = {};
    model.gvrange = gvrange;
    model.gvrate = gvrate;

    $.ajax({
        type: "POST",
        url: "premiumtpamaster.aspx/SaveRecord",
...
function EditRecordbyID(id) {
    ...
    $.ajax({
        type: "POST",
        url: "premiumtpamaster.aspx/GetRecordbyID",
        data: '{ id :' + id + '}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (result) {
            ...
            if (result.d == "10") {
                ...
                $('#txtName').val(json[0].gvrange);
                $('#txtgvrate').val(json[0].gvrate);
...
function UpdateRecord() {
    ...
    if (id > 0) {
        ...
        $.ajax({
            type: "POST",
            url: "premiumtpamaster.aspx/UpdateRecord",
            data: '{ model :' + JSON.stringify(model) + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function DeleteRecord(id) {
    if (confirm('Are you sure delete this record?')) {
        ...
        $.ajax({
            type: "POST",
            url: "premiumtpamaster.aspx/DeleteRecord",
            data: '{ id :' + id + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
...
function validate() {
    ...
    if ($('#txtgvrate').val().trim() == "") {
        $('#txtgvrate').css('border-color', 'Red');
        isValid = false;
    }
    else {
        $('#txtgvrate').css('border-color', 'green');
    }
