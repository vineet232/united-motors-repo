...
//////// $.ajax({
////////        type: "POST", url: "inventorylist.aspx/GetFinanceCompanyName",
////////        dataType: "json",
...
////////    $.ajax({
////////        type: "POST", url: "inventorylist.aspx/GetModelName",
////////        dataType: "json",
////////        contentType: "application/json", 
////////        success: function (res) {
////////            $.each(res.d, function (data, value) {
 ...
 function GetPartDetailsbyPartNo()
 {
   ...
   //// alert(partNo);
    ...
    $("#Showoverlay").show();
     $.ajax({
         type: "POST",
         url: "inventorylist.aspx/GetPartDescByPartNo",
         ///////data: '{ partno :' + partNo + '}',
...
function BindDataTable() {
 ...
 $("#tblRecords").dataTable().fnDestroy();
    ...
    { "visible": false, "targets": [] }
                ...
                bServerSide: true,
                sAjaxSource: 'inventoryListHandler.ashx/GetAllList',
                sServerMethod: 'post',
                "processing": true,
                 ...
                 { "data": "days" },
                     ...
                     
                   {
                       "data": "id",
                        render: function ( data, type, row ) {
                           //debugger
                           return '<a href="#" onclick="ViewRecordbyID(\'' + row.PartNo + '\'\)"><i class="glyphicon glyphicon-eye-open text-primary" style="font-size:x-large;"></i></a>'
                       }
                   }
                     ...
                     
                   {
                       "data": "id",
                        render: function ( data, type, row ) {
                           //debugger
                           return '<a href="#" onclick="ViewBackOrderDetailsbyID(\'' + row.PartNo + '\'\)"><i class="glyphicon glyphicon-envelope text-warning" style="font-size:x-large;"></i></a>'
                       }
                   }
...
////    model.lastreceived = lastreceived;
    ...
    $.ajax({
        type: "POST",
        url: "inventorylist.aspx/SaveRecord",
        data: '{ model :' + JSON.stringify(model) + '}',
...
function EditRecordbyID(id) {
    ...
    $.ajax({
        type: "POST",
        url: "inventorylist.aspx/GetRecordbyID",
        data: '{ id :' + id + '}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
//        model.qty = qty;
        ...
        $.ajax({
            type: "POST",
            url: "inventorylist.aspx/UpdateRecord",
            data: '{ model :' + JSON.stringify(model) + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
            ...
            if (r.d == 0) {
                ...
                if (r.d == 10 || r.d == 20 || r.d == 30  || r.d == 60 || r.d == 70) {
                    if (r.d == 10) {
                    ...
                    if (r.d == 30) {
                      ...
                      if (r.d == 70) {
                        $.notify("Error!! Balance quantity must be greater than 0!", "error");
                        $('[id*=txtPartNo]').focus();
                        return;
                    }
                }
...
//        table.row($(this).parents('tr')).remove().draw();
        ...
        $.ajax({
            type: "POST",
            url: "inventorylist.aspx/DeleteRecord",
            data: '{ id :' + id + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
...
function ViewRecordbyID(partno)
{
    

    $('#myModal2').modal('show');

    ShowTransactionDetails(partno)
   
}


function ShowTransactionDetails(partno) {
    ...
    $.ajax({
        type: "POST",
        url: "inventorylist.aspx/GetTransactionDetailsbyPartNo",
        data: "{ model :" + JSON.stringify(model) + "}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {

            $("#tblRecords2").dataTable().fnDestroy();
            var json = JSON.parse(data.d)
           
            var rows = '';
            $('#tblRecords2 tbody').empty();
            var srno = 0;
                ...
                "<td>" + item.saleqty + ' ' +
                    ...
                    "<td> " +
                     item.createdby + ' (' + moment(item.createdon).format('DD-MMM-YYYY hh:mm A') + ')  <br/>' + item.updatedby + ' (' + moment(item.updatedon).format('DD-MMM-YYYY hh:mm A') + ')'  
                      ...
                      "<td>" + item.invoiceslipno +  '<hr /><br/> Vendor: '  + item.vendorname +  '<hr /><br/> Last Purchased Price: ' + "<span ><i class='fa fa-rupee '></i></span> &nbsp;" + item.purchaseprice + ' ' + "</td>" +
                      "<td>" + item.vehicleno + ' ' + "</td>" +
                      
                 + "</tr>";
            });
           
            $('#tblRecords2 tbody').append(rows);

            // Apply DataTable Plugin.
            table = $('[id*=tblRecords2]').DataTable({
                "order": [[1, "asc"]],
                "columnDefs": [
                                { "visible": false, "targets": [] }
                              ],
...
function ViewBackOrderDetailsbyID(partno)
{
    

    $('#myModal3').modal('show');

    ShowBackOrderDetails(partno)
   
}


function ShowBackOrderDetails(partno) {
   ...
   //// $("#Showoverlay").show();
    ...
    $.ajax({
        type: "POST",
        url: "inventorylist.aspx/GetBackOrderDetailsbyPartNo",
        data: "{ model :" + JSON.stringify(model) + "}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",       
        success: function (data) {
         debugger;
            $("#tblBackOrderDetails").dataTable().fnDestroy();
            var json = JSON.parse(data.d)
           
            var rows = '';
            $('#tblBackOrderDetails tbody').empty();
            var srno = 0;
            $.each(json, function (key, item) {
                ...
                "<td>" + item.Qty + ' ' +
                    ...
                    "<td> " +
                     ///item.createdby + ' (' + moment(item.o).format('DD-MMM-YYYY hh:mm A') + ')  <br/>' + item.updatedby + ' (' + moment(item.updatedon).format('DD-MMM-YYYY hh:mm A') + ')'  
                     item.DiscPer
                      + "</td>" +
            ...
            $('#tblBackOrderDetails tbody').append(rows);

            // Apply DataTable Plugin.
            table = $('[id*=tblBackOrderDetails]').DataTable({
                "order": [[1, "asc"]],
                "columnDefs": [
                                { "visible": false, "targets": [] }
                              ],
