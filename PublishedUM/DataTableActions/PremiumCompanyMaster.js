...
var show = false;
 ...
 var buttons = [''];
  ...
  if (show) {
    ...
    $.ajax({
        type: "POST",
        url: "premiumcompanymaster.aspx/GetAllData",
        data: '',
        ...
        success: function (data) {
            ...
            $.each(json, function (key, item) {
                srno += 1;
                //var id = item.id;
                //var pcompanyname = item.pcompanyname;
               
                rows += "<tr>" +
                "<td>" + item.id + "</td>" +
                "<td>" + srno + "</td>" +
                "<td>" + item.pcompanyname + ' ' +
                    "</td>" +
                      "<td>" + item.parate + ' ' +
                    "</td>" +
                    "<td> " +
...
function AddRecord() {
    ...
    var pcompanyname = $('[id*=txtName]').val();
    var parate = $('[id*=txtparate]').val();
   // name = sentenceCase(name);
    var model = {};
    model.pcompanyname = pcompanyname;
    model.parate = parate;

    $.ajax({
        type: "POST",
        url: "premiumcompanymaster.aspx/SaveRecord",
        ...
        success: function (r) {
            ...
            if (r.d == 10) {
                ...
                if (r.d == 20 || r.d == 30) {
                    ...
                    if (r.d == 30) {
                        $.notify("Error!! Field cannot be blank!", "error");
                        $('[id*=txtName]').focus();
                        return;
                    }
                }
...
function EditRecordbyID(id) {
    ...
    $.ajax({
        type: "POST",
        url: "premiumcompanymaster.aspx/GetRecordbyID",
        data: '{ id :' + id + '}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (result) {
            ...
            if (result.d == "10") {
                ...
                $('#txtName').val(json[0].pcompanyname);
                $('#txtparate').val(json[0].parate);
...
function UpdateRecord() {
    ...
    if (id > 0) {
        ...
        $.ajax({
            type: "POST",
            url: "premiumcompanymaster.aspx/UpdateRecord",
            data: '{ model :' + JSON.stringify(model) + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function DeleteRecord(id) {
    if (confirm('Are you sure delete this record?')) {
        ...
        $.ajax({
            type: "POST",
            url: "premiumcompanymaster.aspx/DeleteRecord",
            data: '{ id :' + id + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
...
function validate() {
    ...
    if ($('#txtparate').val().trim() == "") {
        $('#txtparate').css('border-color', 'Red');
        isValid = false;
    }
    else {
        $('#txtparate').css('border-color', 'green');
    }
