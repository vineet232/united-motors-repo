...
//    $.ajax({
//        type: "POST",
//        url: "mastervendorsVA.aspx/GetAllData",
//        data: '',
...
////                bServerSide: true,
////                sAjaxSource: 'MasterVendorsHandlerVA.ashx/GetVendorsList',
////                sServerMethod: 'post',
////                "processing": true,
////                
                    ...
                    { "data": "oisactive",
                         ...
                         render: function ( data, type, row )
                         {
                             ...
                             var statusdate = moment(row.statusupdatedon).format('DD-MMM-YYYY hh:mm A')
                               if (status == "1") 
                               {                               
                                    return '<a href="#" onclick="updatestatus(' + row.id + ')"><i class="glyphicon glyphicon-ok text-success" style="font-size:x-large;"></i></a>'
                                     + '<br /><br /> <div style="font-size:xx-small;">(' + row.statusupdatedby + '<br/>' + statusdate + ')</div>';;
                                }
                                else
                                {
                                 return  '<a href="#" onclick="updatestatus(' + row.id + ')"><i class="glyphicon glyphicon-remove text-danger" style="font-size:x-large;"></i></a>'
                                  + '<br /><br /><div style="font-size:xx-small;">(' + row.statusupdatedby + '<br/>' + statusdate + ')</div>';;
                                }
                       }
                     },
...
function updatestatus(id)
 {
     ...
     $("#Showoverlay").show();
        $.ajax({
            type: "POST",
            url: "mastervendorsVA.aspx/UpdateStatus",
            data: '{ id :' + id + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
...
function AddRecord() {
   ...
   // name = sentenceCase(name);
    ...
    $.ajax({
        type: "POST",
        url: "mastervendorsVA.aspx/SaveRecord",
...
function EditRecordbyID(id) {
    ...
    $.ajax({
        type: "POST",
        url: "mastervendorsVA.aspx/GetRecordbyID",
        data: '{ id :' + id + '}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (result) {
            ...
            if (result.d == "10") {
                ...
                ////alert("123");
                 ...
                 var opbaldate = moment(json[0].openingbaldate).format('DD-MMM-YYYY')
...
function UpdateRecord() {
    ...
    if (id > 0) {
        ...
        $.ajax({
            type: "POST",
            url: "mastervendorsVA.aspx/UpdateRecord",
            data: '{ model :' + JSON.stringify(model) + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function DeleteRecord(id) {
    if (confirm('Are you sure delete this record?')) {
       ...
       ///// table.row($(this).parents('tr')).remove().draw();
        ...
        $.ajax({
            type: "POST",
            url: "mastervendorsVA.aspx/DeleteRecord",
            data: '{ id :' + id + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
