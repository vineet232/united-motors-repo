<%@ WebHandler Language="C#" Class="InventoryListHandler" %>
...
public class InventoryListHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        List<InventoryListFunctions> listData = new List<InventoryListFunctions>();
        int filteredCount = 0;
        ...
        {
            SqlCommand cmd = new SqlCommand("spGetDataForInventoryList", con);
            ...
            while (rdr.Read())
            {
                InventoryListFunctions listItems = new InventoryListFunctions();
                ...
                InventoryFunctions obj = new InventoryFunctions();
                obj.partno = rdr["PartNo"].ToString().Trim();
                DataTable dtcurrentStockBalance = InventoryFunctions.GetInventoryStockReportwithReturnbyPartNo(obj);
               ...
               //// decimal availQty = decimal.Parse(rdr["qty"].ToString());
                ...
                for (int i = dtcurrentStockBalance.Rows.Count - 1; i >= 0; i--)
                {
                   ...
                   /// 
                    ...
                    if (float.Parse(dtcurrentStockBalance.Rows[i]["purchaseqty"].ToString()) > 0 && dtcurrentStockBalance.Rows[i]["invoiceslipno"].ToString().ToLower() != "opening stock")
                    {
    ...
    private int GetRecordsTotalCount()
    {
        ...
        {
            SqlCommand cmd = new
                SqlCommand("select count(*) from tblInventoryList", con);
            con.Open();
