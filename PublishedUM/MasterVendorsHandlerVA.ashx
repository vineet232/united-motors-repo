<%@ WebHandler Language="C#" Class="MasterVendorsHandlerVA" %>
...
public class MasterVendorsHandlerVA : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        List<VendorMasterFunctionsVA> listData = new List<VendorMasterFunctionsVA>();
        int filteredCount = 0;
        ...
        {
            SqlCommand cmd = new SqlCommand("spGetDataVendorMasterVA", con);
            ...
            while (rdr.Read())
            {
                ...
                VendorMasterFunctionsVA listItems = new VendorMasterFunctionsVA();
                ...
                if (rdr["id"].ToString().Trim() == "12")
                {
                    string abd = rdr["id"].ToString();

                }
                DataSet dsOpBal = ReportsFunctions.GetVendorOpeningBalanceVA(rdr["id"].ToString());

                string openingBal = dsOpBal.Tables[0].Rows[0]["openingbal"].ToString();

                DataSet dsCurrentBal = ReportsFunctions.GetVendorCurrentBalanceVA(rdr["id"].ToString());
                ...
                if (currentBal > 0)
                {
                    ...
                    DataSet dsPurchase = VendorMasterFunctionsVA.GetVendorsPurchaseTransactionByVendorID(rdr["id"].ToString());
                    DataSet dsPayment = VendorMasterFunctionsVA.GetVendorsPaymentTransactionByVendorID(rdr["id"].ToString());
    ...
    private int GetRecordsTotalCount()
    {
        ...
        {
            SqlCommand cmd = new
                SqlCommand("select count(*) from mastervendorva", con);
            con.Open();
