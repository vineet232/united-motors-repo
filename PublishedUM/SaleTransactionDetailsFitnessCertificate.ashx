<%@ WebHandler Language="C#" Class="SaleTransactionDetailsFitnessCertificate" %>
...
public class SaleTransactionDetailsFitnessCertificate : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        {
            SqlCommand cmd = new SqlCommand("spGetDataForSaleTransactionFitness", con);
            ...
            if (searchfieldparameters.Trim() == "")
            {
                ...
                searchvalueparameters2 = "0";
            }

            if (searchfieldparameters.Trim() != "invoicedt")
            {
                searchvalueparameters2 = "0";
            }
            ...
            SqlParameter SearchValueParameters2 = new SqlParameter()
            {
                ParameterName = "@searchvalue2",
                Value = searchvalueparameters2
            };
            cmd.Parameters.Add(SearchValueParameters2);
            ...
            while (rdr.Read())
            {
                ...
                listItems.newcustomername = rdr["newcustomername"].ToString();
                listItems.newmobileno = rdr["newmobileno"].ToString();
                listItems.newmodelid = rdr["newmodelid"].ToString();
                listItems.newmodel = rdr["newmodel"].ToString();
                listItems.newinsurancedt = rdr["newinsurancedt"].ToString();
                listItems.newfitnessdt = rdr["newfitnessdt"].ToString();
                listItems.isfitnessfileuploaded = rdr["isfitnessfileuploaded"].ToString();
    ...
    private int GetRecordsTotalCount()
    {
        ...
        {
            SqlCommand cmd = new
                SqlCommand("select count(*) from salesTransactionDetailsCustomers    where  invoicetypeid=7", con);
            con.Open();
