<%@ WebHandler Language="C#" Class="paymenttransactionsHandler" %>
...
public class paymenttransactionsHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("spGetDataForPaymentTransactionNew", con);
            ...
            while (rdr.Read())
            {
                ...
                listItems.isspecialremarks = rdr["isspecialremarks"].ToString();
                
                if (Convert.ToString(context.Session["loggedmobileno"]) == "9896601858" || Convert.ToString(context.Session["loggedmobileno"]) == "9466730050" )
                {
                    listItems.remarks = rdr["remarks"].ToString();
                }
                else
                {
                    if (rdr["isspecialremarks"].ToString().Trim() == "1")
                    {
                        listItems.remarks = "";
                    }
                    else
    ...
    private int GetRecordsTotalCount()
    {
        ...
        {
            SqlCommand cmd = new
                SqlCommand("select count(*) from paymentTransactionDetails ", con);
            con.Open();
