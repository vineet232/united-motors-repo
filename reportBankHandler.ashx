<%@ WebHandler Language="C#" Class="reportBankHandler" %>
...
public class reportBankHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    double totalRunningBal = 0;
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        string getrunningTotal = context.Request["runningtotal"];
        ...
        {
            SqlCommand cmd = new SqlCommand("spGetDataForBankReport", con);
            ...
            float paid = 0;
            float received = 0;
            double runningDiff = 0;
            float opBalance = 0;
            ///////////// to add opening balance //////////////
            ReportsFunctions OBlistItems = new ReportsFunctions();
            DataSet ds = ReportsFunctions.GetBanksOpeningBalance(bankID);
            ...
            ////DataSet dsOpBal = ReportsFunctions.GetBanksOpeningBalance(rdr["id"].ToString());
            ...
            while (rdr.Read())
            {
                ...
                try
                {
                    runningDiff += double.Parse(rdr["amountreceived"].ToString()) - double.Parse(rdr["amountpaid"].ToString());
                }
                catch { }

                /////totalRunningBal += runningDiff;

                runningDiff += double.Parse(getrunningTotal);
                ...
                listItems.RunningBalance = Convert.ToString(Math.Round((decimal.Parse(runningDiff.ToString())), 2));

               //// context.Session["runningBal"] = runningDiff;
                listData.Add(listItems);
            }


            ////////if (!string.IsNullOrEmpty(context.Session["runningBal"].ToString()))
            ////////{
            ////////    totalRunningBal += double.Parse(context.Session["runningBal"].ToString());
            ////////}
    ...
    private int GetRecordsTotalCount()
    {
        ...
        {
            SqlCommand cmd = new
                SqlCommand("select count(*) from tmpBankReport ", con);
            con.Open();
